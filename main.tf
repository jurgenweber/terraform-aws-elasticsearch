...
resource "aws_iam_role" "elasticsearch_user" {
  count              = module.this.enabled && var.create_iam_user ? 1 : 0
...
data "aws_iam_policy_document" "assume_role" {
  count = module.this.enabled && (length(var.iam_authorizing_role_arns) > 0 || length(var.iam_role_arns) >0 || var.create_iam_user) ? 1 : 0
